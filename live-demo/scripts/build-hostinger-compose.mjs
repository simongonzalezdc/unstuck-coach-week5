...
function cl(x,n=2500){return String(x||"").replace(/\s+/g," ").trim().slice(0,n)}
function hi(a){return Array.isArray(a)?a.flatMap(x=>{const role=x?.role==="assistant"?"assistant":x?.role==="user"?"user":"";const content=cl(x?.content,1e3);return role&&content?[{role,content}]:[]}).slice(-6):[]}
...
const H={};let A=0;function ck(q){let o=q.headers.origin,k=(q.headers["x-forwarded-for"]||q.socket.remoteAddress||"").split(",").pop().trim(),b=H[k]||[0,0],n=Date.now();if(o&&new URL(o).host!==q.headers.host)throw Error("403 origin");if(n>b[1])b=[0,n+6e5];if(++b[0]>12)throw Error("429 rate");H[k]=b;if(A>1)throw Error("429 busy")}
C(async(q,r)=>{try{const u=new URL(q.url,"http://x");if(q.method==="GET"&&u.pathname==="/api/config")return S(r,200,{model:process.env.OPENAI_MODEL});if(q.method==="GET"&&u.pathname==="/"){mk("view");r.writeHead(200,{"content-type":"text/html"});return r.end(page)}if(q.method==="POST"&&u.pathname==="/api/coach"){ck(q);if(!/json/i.test(q.headers["content-type"]||""))throw Error("415 json");const b=await bd(q),msg=cl(b.message),ctx=cl(b.context,800);if(!msg)return S(r,400,{error:"message required"});mk("chat");const p={model:process.env.OPENAI_MODEL,max_tokens:1200${thinking},messages:[{role:"system",content:await cc()},...hi(b.history),{role:"user",content:ctx?msg+"\n\nContext:\n"+ctx:msg}]};let rr;A++;try{rr=await fetch(process.env.OPENAI_BASE_URL.replace(/\/$/,"")+"/chat/completions",{method:"POST",headers:{authorization:"Bearer "+process.env.OPENAI_API_KEY,"content-type":"application/json"},signal:AbortSignal.timeout(25e3),body:JSON.stringify(p)})}finally{A--}if(!rr.ok)throw Error("llm");const j=await rr.json(),text=j.choices?.[0]?.message?.content?.trim();if(!text)throw Error("no text");mk("reply");return S(r,200,{reply:sh(text)})}const api=u.pathname.startsWith("/api/");S(r,api?405:404,{error:api?"bad method":"not found"})}catch(e){mk("error");const m=/^(\d+) (.+)/.exec(e.message);S(r,m?+m[1]:502,{error:m?m[2]:"demo failed"})}}).listen(process.env.PORT||3000,"0.0.0.0");`;
