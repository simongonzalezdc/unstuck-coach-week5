...
const provider = isZaiCodingPlan ? "zai-coding-plan" : "openai-compatible";
const providerLabel = isZaiCodingPlan ? "Z.AI GLM-5.1 (medium reasoning)" : "VPS local LLM";
const baseUrl = isZaiCodingPlan ? "https://api.z.ai/api/coding/paas/v4" : "http://llama-local:8085/v1";
const apiKey = isZaiCodingPlan ? "${ZAI_API_KEY:?ZAI_API_KEY required}" : "local";
...
async function cc(){if(M)return M;let p=[];for(const[f,n]of F){const r=await fetch(process.env.CONTEXT_BASE_URL+"/"+f);if(!r.ok)throw Error("context "+f);p.push((await r.text()).slice(0,n))}return M=("You are Unstuck Coach. One reflection, one next move, one tiny answer. Do not print labels. Choose the move; do not ask user to pick. Use prior turns. First cold prompt: ask for messy pile or any three items.\n"+p.join("\n")).slice(0,8000)}
function S(r,c,o){r.writeHead(c,{"content-type":"application/json"});r.end(JSON.stringify(o))}
async function bd(q){let s="";for await(const c of q){s+=c;if(s.length>32000)throw Error("413 request body too large")}if(!s)return{};try{return JSON.parse(s)}catch{throw Error("400 invalid JSON body")}}
function cl(x){return String(x||"").replace(/\s+/g," ").trim().slice(0,4000)}
...
function mk(x){console.log("unstuck_evt "+x+" "+Date.now())}
const page=B(Buffer.from(${JSON.stringify(pageBr)},"base64")).toString();
C(async(q,r)=>{try{const u=new URL(q.url,"http://x");if(q.method==="GET"&&u.pathname==="/api/config")return S(r,200,{provider:"${provider}",providerLabel:"${providerLabel}",model:process.env.OPENAI_MODEL});if(q.method==="GET"&&u.pathname==="/"){mk("view");r.writeHead(200,{"content-type":"text/html"});return r.end(page)}if(q.method==="POST"&&u.pathname==="/api/coach"){const b=await bd(q),msg=cl(b.message),ctx=cl(b.context);if(!msg)return S(r,400,{error:"message is required"});mk("chat");const p={model:process.env.OPENAI_MODEL,temperature:.1,max_tokens:1200${thinking},messages:[{role:"system",content:await cc()},...hi(b.history),{role:"user",content:ctx?msg+"\n\nSupplied visible context:\n"+ctx:msg}]};const rr=await fetch(process.env.OPENAI_BASE_URL.replace(/\/$/,"")+"/chat/completions",{method:"POST",headers:{authorization:"Bearer "+process.env.OPENAI_API_KEY,"content-type":"application/json"},signal:AbortSignal.timeout(25000),body:JSON.stringify(p)});if(!rr.ok)throw Error("LLM HTTP "+rr.status+": "+(await rr.text()).slice(0,160));const j=await rr.json(),text=j.choices?.[0]?.message?.content?.trim();if(!text)throw Error("LLM response had no text");mk("reply");return S(r,200,{reply:sh(text)})}const api=u.pathname.startsWith("/api/");S(r,api?405:404,{error:api?"method not allowed":"not found"})}catch(e){mk("error");const m=/^(\d+) (.+)/.exec(e.message);S(r,m?+m[1]:502,{error:m?m[2]:"live demo failed"})}}).listen(process.env.PORT||3000,"0.0.0.0");`;
